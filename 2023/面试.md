...
2. 协商缓存可以通过last-modified / if-modified-since 和 etag / if-none-match 这两对header来控制
   ...
   优先选择 ETag，原因如下：
精度更高，Last-Modified 只精确到秒，1 秒内多次修改无法识别；ETag 基于文件内容哈希，内容变化就会变化。
不会误判，文件仅修改权限、时间，内容不变时，Last-Modified 会误判更新，ETag 不受修改时间影响。
适用范围更广，不仅静态资源，后端动态接口、动态数据也可以使用 ETag。
分布式多服务器部署场景，多机文件修改时间不一致会导致Last-Modified错乱，ETag 内容一致则标识一致，集群更稳定。
唯一不足是 ETag 计算哈希性能开销略高，但现代服务器可以忽略。并且浏览器协商缓存优先级也是 ETag 高于 Last-Modified。
