...
2. 协商缓存可以通过last-modified / if-modified-since 和 etag / if-none-match 这两对header来控制
   ...
   ```
   If-Modified-Since：当缓存资源过期时且具有last-modified声明，则带上If-Modified-Since，值为last-modified,web服务器收到请求猴发现请求头有if-modified-since的话，则与被请求的资源最后修改时间进行比对，若最后修改时间较旧，则说明资源无新修改，反之则资源被修改了
